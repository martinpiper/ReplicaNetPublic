...
CD3DMesh*	 gpProjectileMesh;
CD3DMesh*	 gpEnemyMesh;
char *gCommandLine;
...
CMyD3DApplication *mSingleton = 0;
...
CMyD3DApplication *CMyD3DApplication::mSingleton = 0;
...
{
	m_strWindowTitle	= _T("Example4: Pilot the planes");
...
//	m_bUseDepthBuffer   = FALSE;
	...
	gpProjectileMesh = new CD3DMesh();
	gpEnemyMesh = new CD3DMesh();
...
HRESULT CMyD3DApplication::InitDeviceObjects()
{
	...
	if( FAILED( gpEnemyMesh->Create( m_pd3dDevice, _T("dolphin1.x") ) ) )
		return D3DAPPERR_MEDIANOTFOUND;

	if( FAILED( gpProjectileMesh->Create( m_pd3dDevice, _T("cube.x") ) ) )
		return D3DAPPERR_MEDIANOTFOUND;
	...
	gpProjectileMesh->SetFVF( m_pd3dDevice, D3DFVF_VERTEX );
	m_pTerrainObject->SetFVF( m_pd3dDevice, D3DFVF_VERTEX );
	gpEnemyMesh->SetFVF( m_pd3dDevice, D3DFVF_VERTEX );
...
HRESULT CMyD3DApplication::RestoreDeviceObjects()
{
	...
	gpProjectileMesh->RestoreDeviceObjects( m_pd3dDevice );
	m_pTerrainObject->RestoreDeviceObjects( m_pd3dDevice );
	gpEnemyMesh->RestoreDeviceObjects( m_pd3dDevice );
...
HRESULT CMyD3DApplication::InvalidateDeviceObjects()
{
	...
	gpProjectileMesh->InvalidateDeviceObjects();
	m_pTerrainObject->InvalidateDeviceObjects();
	gpEnemyMesh->InvalidateDeviceObjects();
	return S_OK;
}
...
CMyD3DApplication *CMyD3DApplication::GetCMyD3DApplication(void)
{
	return mSingleton;
}
